...
class GEN():
    def __init__(self, funtion, population, cant_genes = None, cant_ciclos= None, selection_percent = None, 
                 crossing = None, mutation_percent = None, i_min = None, i_max = None, optimum = None, num_qubits = None):
        # Guardamos los valores una sola vez
...
class scl_alg_stn_bin_rank(GEN):
    def __init__(self, funtion, population, cant_genes, cant_ciclos, 
                 selection_percent, crossing, mutation_percent, i_min, 
                 i_max, optimum):
        super().__init__(funtion, population, cant_genes, cant_ciclos, 
                         selection_percent, crossing, mutation_percent, 
                         i_min, i_max, optimum)
...
class cl_alg_quantum(GEN):
    def __init__(self, funtion, population, cant_ciclos, crossing, mutation_percent, i_min, 
                 i_max, optimum, num_qubits):
        super().__init__(funtion, population, cant_ciclos, crossing, mutation_percent, i_min,
                         i_max, optimum, num_qubits)


    def run(self):
        print(f"\n[INFO] Starting algorithm: Cuantitative for Rank")
        self.bin_population = self.create_binary_population(self.population, self.cant_genes)
        for i in range(self.cant_ciclos):
            self.Crossing_bin = self.crossing_binary_population(self.bin_population, self.crossing)
            self.Mutations_bin = self.mutations_binary_population(self.Crossing_bin, self.mutation_percent)
    ...
    def quantum_amplitudes(self, population, num_qubits):
        amplitudes = np.full((population.shape[0], num_qubits), 1 / np.sqrt(2))
        return amplitudes
